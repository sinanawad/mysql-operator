    ...
    def uninstall_mysql() -> None:
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                subprocess.run(["/usr/bin/snap", "remove", "charmed-mysql"], check=True)
                return
            except subprocess.CalledProcessError:
                # uninstalls fail due to SNAP_DATA_DIR fails to umount
                # try umount it, without check
                # Subprocess params are generated by the charm
                subprocess.run(["/usr/bin/umount", CHARMED_MYSQL_COMMON_DIRECTORY])  # noqa: S603
    ...
    def _run_mysqlcli_script(
        ...
        try:
            if password:
                ...
                return [line.rstrip("\r\n").split("\t") for line in stdout[1:]] if stdout else []
            else:
                # Input generated by the charm
